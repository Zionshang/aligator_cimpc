...
void Interpolator::interpolateConfiguration(const double delay, const std::vector<double> &timesteps,
                                            const std::vector<VectorXd> &qs, VectorXd &q_interp)
{
  assert(("Configuration is not of the right size", qs[0].size() == model_.nq));
  assert(("Timesteps size does not match trajectory size", timesteps.size() == qs.size() - 1));

  // Compute the time knot corresponding to the current delay
  double accumulated_time = 0.0;
  size_t step_nb = 0;
  for (; step_nb < timesteps.size(); ++step_nb)
  {
    if (accumulated_time + timesteps[step_nb] > delay)
      break;
    accumulated_time += timesteps[step_nb];
  }

  // Interpolate
  if (step_nb >= timesteps.size())
  {
    q_interp = qs.back();
  }
  else
  {
    double step_progress = (delay - accumulated_time) / timesteps[step_nb];
    q_interp = pinocchio::interpolate(model_, qs[step_nb], qs[step_nb + 1], step_progress);
  }
}

void Interpolator::interpolateState(const double delay, const std::vector<double> &timesteps,
                                    const std::vector<VectorXd> &xs, VectorXd &x_interp)
{
  assert(("State is not of the right size", xs[0].size() == model_.nq + model_.nv));
  assert(("Timesteps size does not match trajectory size", timesteps.size() == xs.size() - 1));
...
void Interpolator::interpolateLinear(const double delay, const std::vector<double> &timesteps,
                                     const std::vector<VectorXd> &vs, VectorXd &v_interp)
{
  assert(("Timesteps size does not match trajectory size", timesteps.size() == vs.size() - 1));
